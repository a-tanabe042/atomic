...
@section('title', 'スキル（DB）一覧')
@include('layouts.head')

@section('content')
    <div class="list">
        <h4 class="text-color-light-blue font-weight-bold">
            {{ $skillCategory['category'] }}一覧
        </h4>
        <div class="skill-add">
            <form action="{{ route('list.store') }}" method="post" id="skillAdd" class="checkform skill-add">
                @csrf
                ...
                <tbody>
                    <tr class="skill-title">
                        <td class="skill-no">No.</td>
                            @if (!is_null($skillCategory['subCategory']))
                                <td class="skill-name">{{ $skillCategory['subCategory'] }}</td>
                            @endif
                        <td class="skill-name">{{ $skillCategory['category'] }}</td>
                        <td class="skill-no"></td>
                        <td class="skill-no"></td>
                    </tr>
                    @foreach ($skillList as $skill)
                        <form class="update" action="{{ route('list.update', ['skill_id' => $skill->skill_id]) }}"
                            ...
                            <tr id="skillAdd" style="display:none" class="color1 tr-add-{{ $loop->iteration }}">
                                ...
                                @if (!is_null($skillCategory['subCategory']))
                                <td class="skill-name">
                                    <input type="text" class="form-input" name="sub_class_name" placeholder="{{ $skill->sub_class_name }}">
                                </td>
                                @endif
                                <td class="skill-name">
                                    <input type="text" class="form-input" name="skill_name" placeholder="{{ $skill->skill_name }}">
                                </td>
                        ...
                        <tr id="skillList" style="" class="color1 tr-list-{{ $loop->iteration }}">
                            <td class="skill-no">{{ $loop->iteration }}</td>
                            @if (!is_null($skillCategory['subCategory']))
                            <td class="skill-name">{{ $skill->sub_class_name }}</td>
                            @endif
                            <td class="skill-name">{{ $skill->skill_name }}</td>
                            ...
                            <td class="skill-update">
                                <form id="del-skill" action="{{ route('list.destroy', ['skill_id' => $skill->skill_id]) }}"
                                    method="POST">
                                    @csrf
                                    @method('DELETE')
    ...
    <script type="text/javascript">
        ...
        $(function() {
            ...
            $('button#add').click(function() {
                var category = "{{ $skillCategory['category'] }}";
                var subCategory =  "{{ $skillCategory['subCategory'] }}";
                var tr_form = '<tr>';

                if (subCategory !== "") {
                    tr_form +=
                        '<td><input type="text" name="sub_class_name[]" placeholder="' + subCategory +
                    '"></td>';
                }

                tr_form += '<td><input type="text" name="skill_name[]" placeholder="' + category +
                    '"></td>' +
                    '</tr>';
