...
@section('content')
    <div class="list">
        ...
        <div class="skill-add">
            <form action="{{ route('add.os') }}" method="post" id="skillAdd" class="checkform skill-add">
                @csrf
            ...
            <table class="skill-list" border="1">
                <tbody>
                    <tr class="skill-title">
                        ...
                        <td class="skill-name">OS</td>
                        <td class="skill-no"></td>
                        <td class="skill-no"></td>
                    </tr>
                    @foreach ($osList as $os)
                        <form class="update" action="{{ route('update.skill', ['skill_id' => $os->skill_id]) }}"
                        ...
                        <tr id="skillList" style="" class="color1 tr-list-{{ $loop->iteration }}">
                            <td class="skill-no">{{ $loop->iteration }}</td>
                            <td class="skill-name">{{ $os->skill_name }}</td>
                            ...
                            <td class="skill-update">
                                <form id="del-skill" action="{{ route('skill.destroy', ['skill_id' => $os->skill_id]) }}"
                                    method="POST">
                                    @csrf
