...
@section('content')
    <div class="list">
        ...
        <div class="skill-add">
            <form action="{{ route('add.platform') }}" method="post" id="skillAdd" class="checkform skill-add">
                @csrf
            ...
            <table class="skill-list" border="1">
                <tbody>
                    <tr class="skill-title">
                        ...
                        <td class="skill-name">プラットフォーム</td>
                        <td class="skill-name">サービス</td>
                        <td class="skill-no"></td>
                        <td class="skill-no"></td>
                    </tr>
                    @foreach ($platformList as $platform)
                        <form class="update" action="{{ route('update.skill', ['skill_id' => $platform->skill_id]) }}"
                            ...
                            <tr id="skillAdd" style="display:none" class="color1 tr-add-{{ $loop->iteration }}">
                                ...
                                <td class="skill-name">
                                    <input type="text" class="form-input" name="sub_class_name"
                                        placeholder="{{ $platform->sub_class_name }}">
                                </td>
                                <td class="skill-name">
                                    <input type="text" class="form-input" name="skill_name"
                                        placeholder="{{ $platform->skill_name }}">
                                </td>
                        ...
                        <tr id="skillList" style="" class="color1 tr-list-{{ $loop->iteration }}">
                            <td class="skill-no">{{ $loop->iteration }}</td>
                            <td class="skill-name">{{ $platform->sub_class_name }}</td>
                            <td class="skill-name">{{ $platform->skill_name }}</td>
                            <td class="skill-update"><button type="button" class="btn update-{{ $loop->iteration }}"
                                    id="update" onClick="update(this)">編集</button></td>
                            <td class="skill-no">
                                <form id="del-skill"
                                    action="{{ route('skill.destroy', ['skill_id' => $platform->skill_id]) }}"
                                    method="POST">
                                    @csrf
    ...
    <script type="text/javascript">
        ...
        $(function() {
            ...
            $('button#add').click(function() {
                var tr_form = '' +
                    '<tr>' +
                    '<td><input type="text" name="sub_class_name[]" placeholder="プラットフォーム"></td>' +
                    '<td><input type="text" name="skill_name[]" placeholder="サービス"></td>' +
                    '</tr>';
