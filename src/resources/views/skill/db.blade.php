...
@section('content')
    <div class="list">
        ...
        <div class="skill-add">
            <form action="{{ route('add.db') }}" method="post" id="skillAdd" class="checkform skill-add">
                @csrf
            ...
            <table class="skill-list" border="1">
                <tbody>
                    <tr class="skill-title">
                        ...
                        <td class="skill-name">DB</td>
                        <td class="skill-no"></td>
                        <td class="skill-no"></td>
                    </tr>
                    @foreach ($dbList as $db)
                        <form class="update" action="{{ route('update.skill', ['skill_id' => $db->skill_id]) }}"
                        ...
                        <tr id="skillList" style="" class="color1 tr-list-{{ $loop->iteration }}">
                            <td class="skill-no">{{ $loop->iteration }}</td>
                            <td class="skill-name">{{ $db->skill_name }}</td>
                            ...
                            <td class="skill-update">
                                <form id="del-skill" action="{{ route('skill.destroy', ['skill_id' => $db->skill_id]) }}"
                                    method="POST">
                                    @csrf
