...
@section('content')
    <div class="list">
        ...
        <div class="skill-add">
            <form action="{{ route('add.middleware') }}" method="post" id="skillAdd" class="checkform skill-add">
                @csrf
            ...
            <table class="skill-list" border="1">
                <tbody>
                    <tr class="skill-title">
                        ...
                        <td class="skill-name">ミドルウェア</td>
                        <td class="skill-no"></td>
                        <td class="skill-no"></td>
                    </tr>
                    @foreach ($middlewareList as $middleware)
                        <form class="update" action="{{ route('update.skill', ['skill_id' => $middleware->skill_id]) }}"
                            ...
                            <tr id="skillAdd" style="display:none" class="color1 tr-add-{{ $loop->iteration }}">
                                ...
                                <td class="skill-name">
                                    <input type="text" class="form-input" name="skill_name"
                                        placeholder="{{ $middleware->skill_name }}">
                                </td>
                        ...
                        <tr id="skillList" style="" class="color1 tr-list-{{ $loop->iteration }}">
                            <td class="skill-no">{{ $loop->iteration }}</td>
                            <td class="skill-name">{{ $middleware->skill_name }}</td>
                            ...
                            <td class="skill-update">
                                <form id="del-skill"
                                    action="{{ route('skill.destroy', ['skill_id' => $middleware->skill_id]) }}"
                                    method="POST">
                                    @csrf
